...
foreach($folder in $monitoredFolders)
{
    try {
        ...
        Write-Host "checking" $sftlocation
        ...
        {
            $result = Get-SFTPFile -SessionId ($newSSHSession).SessionId -RemoteFile $file.FullName -LocalPath $folder.destination
        }
        ...
        Write-Host "Disconnecting from" $folder.sourceserver
